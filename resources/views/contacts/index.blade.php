    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card mb-4">
            <div class="card-body d-flex justify-content-between align-items-center">
                ...
                <button class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#addContactModal">Tambah
                    Kontak</button>

                <!-- Filter Kontak -->
                <form id="filter-form" action="{{ route('contacts.index') }}" method="GET" class="d-flex align-items-center"
                    style="margin: 0;">
                    <label for="filter_type" class="form-label mb-0 me-2 text-secondary"
                        style="line-height: 1.5;">Filter:</label>
                    <select name="filter_type" id="filter_type" class="form-control form-control-sm"
                        style="min-width: 200px; height: auto;" onchange="this.form.submit()">
                        <option value="" {{ request('filter_type') == '' ? 'selected' : '' }}>Semua</option>
                        <option value="prolanis kelompok ht 1"
                            {{ request('filter_type') == 'prolanis kelompok ht 1' ? 'selected' : '' }}>Prolanis Kelompok HT
                            1</option>
                        <option value="prolanis kelompok ht 2"
                            {{ request('filter_type') == 'prolanis kelompok ht 2' ? 'selected' : '' }}>Prolanis Kelompok HT
                            2</option>
                        <option value="prolanis kelompok dm"
                            {{ request('filter_type') == 'prolanis kelompok dm' ? 'selected' : '' }}>Prolanis Kelompok DM
                        </option>
        ...
        <div class="card mb-4">
            <div class="card-body d-flex justify-content-between align-items-center">
                <!-- Per Page -->
                <form id="perPage-form" action="{{ route('contacts.index') }}" method="GET"
                    class="d-flex align-items-center" style="margin: 0;">
                    <label for="perPage" class="form-label mb-0 me-2 text-secondary"
                        style="line-height: 1.5;">Tampilkan:</label>
                    <select name="perPage" id="perPage" class="form-control form-control-sm"
                        style="width: 80px; height: auto;" onchange="this.form.submit()">
                ...
                <!-- Search -->
                <form id="search-form" action="{{ route('contacts.index') }}" method="GET"
                    class="d-flex align-items-center" style="margin: 0;">
                    <input type="text" name="search" id="search" class="form-control form-control-sm me-2"
                        placeholder="Cari..." value="{{ request('search') }}" style="max-width: 200px;">
                    ...
                    <tbody style="font-family: 'Verdana', sans-serif; font-size: 12px;">
                        @forelse ($contacts as $index => $contact)
                            <tr style="border-bottom: 1px dashed #ddd;">
                                <td class="text-center" style="color: #000; font-weight: bold;">
                                    {{ ($contacts->currentPage() - 1) * $contacts->perPage() + $index + 1 }}</td>
                                <td>{{ $contact->name }}</td>
                                <td>{{ $contact->phone }}</td>
                                <td>{{ $contact->type }}</td>
                                <td>
                                    ...
                                    <form action="{{ route('contacts.destroy', $contact->id) }}" method="POST"
                                        ...
                                        <button type="submit" class="btn btn-danger btn-sm"
                                            onclick="return confirm('Yakin ingin menghapus kontak ini?')">Hapus</button>
                                    </form>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="text-center text-danger fw-bold"
                                    style="font-family: 'Courier New', monospace; font-size: 14px;">Tidak ada kontak.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
